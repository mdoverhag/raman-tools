...
def plot_reference(averaged_spectrum: dict, molecule: str, output_path: str) -> None:
    """
    ...
    ax.axvline(
        ...
        linewidth=1.5,
        alpha=0.7,
        label=f"Expected peak ({expected_peak} cm⁻¹)",
    )
...
def plot_sample(
    averaged_spectrum: dict, sample_name: str, molecules: list[str], output_path: str
) -> None:
    """
    ...
    for molecule in molecules:
        ...
        ax.axvline(
            ...
            linewidth=1.5,
            alpha=0.7,
            label=f"{molecule} ({expected_peak} cm⁻¹)",
        )
...
def plot_normalization(
        ...
        for (mol, conj), ref_data in reference_spectra.items():
            if mol == molecule:
                ...
                ax1.plot(
                    ...
                    color=color,
                    linewidth=1.5,
                    label=f"{molecule}-{conj} reference",
                    alpha=0.8,
                )
                break
    ...
    # Main title
    fig.suptitle(
        f"Normalization - {sample_name}", fontsize=14, fontweight="bold", y=0.995
    )
...
def plot_deconvolution(
    ...
    ax1.plot(
        wavenumbers, multiplex_data, "k-", linewidth=2, label="Multiplex", alpha=0.7
    )
    ax1.plot(wavenumbers, reconstructed, "r--", linewidth=2, label="Fitted", alpha=0.8)
    ...
    ax1.set_title(
        f"Multiplex Spectrum Deconvolution - {sample_name}",
        fontsize=14,
        fontweight="bold",
    )
    ax1.legend()
    ...
    for mol_conj in sorted(contributions.keys()):
        ...
        ax2.plot(
            ...
            color=color,
            linewidth=2,
            label=f"{molecule}-{conjugate} ({percentage:.1f}%)",
            alpha=0.8,
        )
...
def plot_deconvolution_original_scale(
    ...
    ax1.plot(
        wavenumbers, original_data, "k-", linewidth=2, label="Multiplex", alpha=0.7
    )
    ax1.plot(
        wavenumbers,
    ...
    ax1.set_title(
        f"Multiplex Spectrum Deconvolution - {sample_name} (Original Scale)",
        fontsize=14,
        fontweight="bold",
    )
    ax1.legend()
    ...
    for mol_conj in sorted(contributions.keys()):
        ...
        contribution_original = (
            np.array(individual_contributions[mol_conj]) * norm_factor
        )
...
def plot_deconvolution_boxplots(
    samples: dict, deconv_results: dict, output_path: str
) -> None:
    """
    ...
    for idx, (sample_key, replicate_results) in enumerate(deconv_results.items()):
        ...
        # Create box plot
        bp = ax.boxplot(
            box_data, labels=labels, patch_artist=True, showmeans=True, meanline=True
        )
    ...
    # Main title
    fig.suptitle(
        "Molecular Contributions Distribution Across Replicates",
        fontsize=14,
        fontweight="bold",
        y=0.98,
    )
...
def plot_peak_intensity_histogram(
    ...
    title = f"{molecule} Frequency Distribution at {peak_wn} cm⁻¹"
    if title_prefix:
